...
enctoken = 'i5HcMKgyK11jBPx2QL0UUoHWQVGEgZ1Hr17WjKLR5Iu3q+u+pVh35jcqQ1cHC3oaUw36yJo4NWKCJdZgNVp7TUyfVDaOyA=='
#enctoken = input('enctoken\t')

#params = (('from', datetime.today().strftime('%Y-%m-%d')),('to', datetime.today().strftime('%Y-%m-%d')))
params = (('from', '2021-02-01'),('to', '2021-05-11'))
...
def plot(support, resistance):
        ...
        colors = ['b']*3 + [i for i in 'r'*len(support)] + [j for j in 'g'*len(resistance)]
...
for i in levels_s:
        if i in all_support:
                all_support.append(i-0.1)
        else:
                all_support.append(i)
for i in levels_r:
        if i in all_resistance:
                all_resistance.append(i+0.1)
        else:
